...
void GalleryDlWorker::onReadyReadStandardOutput()
{
    ...
    qsizetype lastNewline = m_outputBuffer.lastIndexOf('\n');
    qsizetype lastCarriageReturn = m_outputBuffer.lastIndexOf('\r');
    qsizetype lastDelimiter = qMax(lastNewline, lastCarriageReturn);
    ...
    for (const QByteArray &line : lines) {
        ...
        if (match.hasMatch()) {
            ...
            if (!filePath.isEmpty() && filePath != m_lastFile) {
                ...
                constexpr qint64 THROTTLE_INTERVAL_MS = 300;
                if (now - lastEmit > THROTTLE_INTERVAL_MS) { // Throttle for webhook/UI updates
...
void GalleryDlWorker::onReadyReadStandardError()
{
    ...
    qsizetype lastCarriageReturn = m_errorBuffer.lastIndexOf('\r');
    qsizetype lastDelimiter = qMax(lastNewline, lastCarriageReturn);
    ...
    for (const QByteArray &line : lines) {
        ...
        if (trimmedLine.startsWith(QStringLiteral("[")) && trimmedLine.contains(QStringLiteral("] "))) {
            ...
            qsizetype lastBracket = trimmedLine.indexOf(QStringLiteral("] "), 0);
            if (lastBracket != -1) {
...
void GalleryDlWorker::onProcessFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    constexpr int EXIT_CODE_PARTIAL_OR_SKIPPED = 101;
    bool success = (exitStatus == QProcess::NormalExit) && (exitCode == EXIT_CODE_SUCCESS || exitCode == EXIT_CODE_PARTIAL_OR_SKIPPED);
    ...
    if (!success && !partialSuccess) {
        ...
        if (exitStatus == QProcess::CrashExit) {
            ...
            errorMsg.append(tr("Process crashed: %1").arg(m_process->errorString()));
        }
        qWarning() << "GalleryDlWorker failed. Exit code:" << exitCode << "Stderr:" << stderrOutput;
        emit finished(m_id, false, tr("gallery-dl failed: %1").arg(errorMsg.isEmpty() ? tr("Unknown error") : errorMsg), QString(), QVariantMap());
        return;
    }
