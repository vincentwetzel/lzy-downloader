...
**Fast Start:** Keep the UI responsive (no blocking I/O on the GUI thread; use `QThread` or `QtConcurrent`). Preserve the download lifecycle (temp -> verify -> move to completed dir). Prefer discovered or user-configured external binaries. Update the maintained docs and `CHANGELOG.md` for significant changes. For releases, bump `CMakeLists.txt` above the newest fetched `v*` tag, create matching release notes, then run `python build_release.py` on the target platform to compile and package; the helper rejects reused versions and mismatched tag builds unless `LZY_ALLOW_VERSION_REBUILD=1` is explicitly set for an intentional rebuild. The helper inherits the invoking terminal and does not launch a separate Windows shell. The NSIS finish page offers to launch the installed `LzyDownloader.exe` after a successful installation. Update Section 3 + Quick-Reference list when files/roles change. For file locations, jump to the Quick-Reference list in Section 3.
...
- **Generic duplicate identity**: Queue, active, paused, retry, and archive checks MUST use the shared normalized media identity rather than raw URL-string equality. Identity normalization may use provider metadata/IDs when available and generic URL normalization otherwise; it MUST NOT add site-specific download behavior.
- **Terminal re-download recovery**: An explicit `override_archive` request may replace a matching restored item only when it is marked stopped/failed; genuinely paused downloads remain protected. Non-interactive duplicate rejections MUST emit the terminal request-failure diagnostic used by the Discord bridge.
- **Retry/resume protection**: Retry and resume requests MUST pass the current active-item snapshot through the queue manager so equivalent active media cannot be re-enqueued during a concurrent state change.
- **Disk-space failures**: `No space left on device`, `ENOSPC`/errno 28, and FFmpeg error `-28` diagnostics MUST be terminal failures. A printed final path MUST NOT allow metadata embedding or finalization to continue after a disk-space failure.
- **Destination replacement**: Explicit re-downloads may replace an existing destination file, but the existing file MUST be preserved until the verified temporary output has been moved or copied successfully.
...
- External-tool update checks use a bounded network watchdog and expose exact
  installed/latest diagnostics persistently for manually managed tools.
...
- **Detailed Progress Display**: The UI MUST display rich, detailed progress information to users during downloads, comparable to command-line yt-dlp output:
  ...
  - When `info.json` omits `requested_downloads`, the worker MUST recover stream sizes from matching `formats` metadata and may poll the owned temporary `.part` file at a bounded interval so a quiet native downloader cannot leave progress stale.
  - Each Active Downloads row MUST show one detailed progress bar for the current transfer or processing stage; no separate aggregate whole-job progress bar is displayed.
  - The Discord webhook may consume the worker's aggregate multi-stream progress fields so bot percentages do not reset when yt-dlp switches between primary streams.
...
**Quick reference:** `src/core/ArchiveManager.cpp` owns normalized media identity comparison and archive persistence. `src/core/DownloadQueueManager.cpp` owns generic duplicate checks across queued, active, paused, retried, and archived items; `src/core/DownloadQueueManagerRecovery.cpp` owns explicit recovery of matching restored stopped/failed entries. `src/core/FileReplacement.cpp` owns recoverable existing-destination replacement while preserving the old file until the new move/copy succeeds. `src/core/YtDlpWorkerDiagnostics.cpp` owns reusable fatal/incomplete-media, disk-space, and bounded aria2c missing-output recovery classification used by yt-dlp output parsing and process completion. `src/core/YtDlpWorker.cpp` owns bounded browser-cookie recovery. `src/core/YtDlpWorkerTransfers.cpp` owns stream-stage inference, including audio extraction from combined sources. `src/core/DownloadManagerWorkers.cpp` owns terminal quality-warning classification and must keep video-only checks separate from audio metadata. `src/core/YtDlpLiveStatus.h` owns the narrow metadata mapping for explicit yt-dlp upcoming/premiere diagnostics used during playlist-probe fallback. `src/core/AppUpdater.cpp` owns platform/CPU-matched release asset selection and Finder-based macOS DMG handoff. `src/core/MetadataEmbedder.cpp` owns the existing FFmpeg rewrite path, including remuxing a tracked abandoned thumbnail as `attached_pic` artwork before cleanup. `src/ui/MainWindowUiBuilder.cpp` owns the compact footer row layout, including keeping the exit-after-downloads control rightmost. The top-level `tests/` directory owns Qt regression tests, shared fixtures, workflow templates, and test-only helper scripts; `tests/run_headless_tests.py` owns timestamped build/test orchestration, fail-fast compilation, final summaries, and the build-local `--suspects` cache; `TestDownloadQueueState.cpp` covers queue-backup serialization/restoration and malformed-entry filtering, `TestDownloadQueueManager.cpp` covers equivalent-URL, retry, and terminal re-download deduplication, `TestFileReplacement.cpp` covers destination preservation, `TestDownloadTempCleanup.cpp` covers root fallback and ownership guards, and `TestYtDlpWorker.cpp` covers positive and negative aria2c recovery plus disk-space failure classification. `README.md` is the public discoverability landing page; `CONTRIBUTING.md` and `SECURITY.md` define contributor and vulnerability-reporting paths; `.github/ISSUE_TEMPLATE/` and `.github/pull_request_template.md` standardize public collaboration.
...
`tools/configure_debug.ps1` owns the VS Code debug-configure recovery path. It
must inspect generated compiler metadata and use `cmake --fresh --preset debug`
only when the standard `build-debug` tree is incomplete or stale.
...
- External binary updates must avoid overwriting package-managed tools directly; prefer package-manager commands or tool-native self-updaters (`yt-dlp -U`, `gallery-dl -U`, `deno upgrade`) surfaced through the External Binaries UI. WinGet package paths under `Microsoft\WinGet\Packages` MUST route through `winget upgrade`; standalone external FFmpeg copies MUST not silently create an app-local replacement.
- An explicit Browse selection or completed app-managed installation is a manual override even when its path is inside the app-local `bin` folder; startup rediscovery MUST NOT replace it with a system/package-manager candidate. Windows FFmpeg/FFprobe paired installs MUST persist both sibling paths with the `.exe` suffix.
- External-tool installation dialogs MUST keep long package-manager and PowerShell command previews within the dialog width by wrapping arbitrary-length tokens and disabling horizontal overflow.
...
- macOS release CI must build separate Intel (`macos-13`) and Apple Silicon (`macos-14`) bundles with Qt's universal `clang_64` desktop archive, run `macdeployqt`, and publish architecture-labelled DMGs. The native release helper accepts `--target auto|windows|linux|macos` and rejects cross-OS targets; the macOS VM may use `python build_release.py --target macos`. The app updater must only select a DMG matching the current CPU architecture; opening a DMG hands installation to Finder rather than attempting to execute it.
