...
- **Local API server**: `src/core/LocalApiServer.h/.cpp` (localhost listener, token auth, enqueue/status endpoints with optional `type` selection and optional caller-provided `id`; routes `api_token.txt` to the `Server/` subfolder when headless/server), `src/ui/advanced_settings/ConfigurationPage.h/.cpp` (`General/enable_local_api` GUI toggle), and `src/ui/MainWindowConnections.cpp`/`src/ui/MainWindowDownloads.cpp` (server lifecycle starts automatically for `--server`/`--headless` without mutating that GUI preference, plus non-interactive enqueue routing).
...
- **Extractor Domain Lists**: `extractors_yt-dlp.json` and `extractors_gallery-dl.json` (copied beside `LzyDownloader.exe` for URL auto-paste). Refresh scripts are non-interactive so release automation and CI do not hang waiting for a final keypress.
