...
- **Livestream Argument Safety**: Active/upcoming livestream downloads MUST avoid arguments that are only safe for finite media files, including SponsorBlock removal, chapter/metadata embedding, thumbnail embedding, and subtitle embedding/writing. Live-state metadata emitted by workers should update active row options so livestream controls such as `Stop & Save` remain accurate after queue creation.
- **Local API CORS Policy**: `LocalApiServer` MUST continue to validate Host and Origin before routing requests. Only localhost and trusted browser-extension origins may receive CORS headers or `204 No Content` preflight responses; unauthorized origins and preflights must be rejected.
