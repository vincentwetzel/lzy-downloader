...
- **Headless helper:** `tests/run_headless_tests.py` builds the selected CMake configuration and runs `ctest` with `QT_QPA_PLATFORM=offscreen` and parallel jobs based on the host CPU count, making GUI-adjacent tests suitable for non-interactive Windows shells and CI runners.
