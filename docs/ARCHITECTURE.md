...
- **GitHub Tag Releases:** The `Build and Release` GitHub Actions workflow triggers on `v*` tag pushes, installs NSIS on `windows-latest`, runs `build_release.py` on `windows-latest` and `ubuntu-22.04`, and attaches both `LzyDownloader-Setup-*.exe` and `LzyDownloader-*-x86_64.AppImage` assets to the tag's GitHub Release.
