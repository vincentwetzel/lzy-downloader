...
- **Auto-paste Control:** `DownloadOptionsPage` saves `General/auto_paste_mode`, and `MainWindow` reacts to app focus/clipboard changes to route matching URLs to `StartTab` according to the selected mode, using a brief debounce plus duplicate queue checks instead of a long lockout. The same page also stores `DownloadOptions/ffmpeg_cut_encoder` and `DownloadOptions/ffmpeg_cut_custom_args`, which let yt-dlp's accurate SponsorBlock cut pass use hardware or custom FFmpeg output arguments. Built-in cut modes also add edit-list and timestamp normalization arguments to prevent MP4 A/V desync after SponsorBlock or section cuts. Hardware encoder choices are populated from asynchronous FFmpeg encoder and local GPU probes so irrelevant options stay hidden.
...
- **External Binaries:** Relies on user-installed binaries (`yt-dlp`, `ffmpeg`, `ffprobe`, `deno`, `gallery-dl`, `aria2c`) found in system paths, the app-local `bin` folder, or configured manually. **`deno` is used as the JavaScript runtime for yt-dlp's YouTube challenge solver (`--js-runtimes deno:...`).** The application includes `SmartBinaryResolver` for version-aware runtime resolution, `ProcessUtils` for process environment and cache helpers, `BaseBinaryUpdater` for shared version/update checks, `BinariesPage` for status/version/install/update UX, and `MissingBinariesDialog` for guided setup when required tools are absent. Manual overrides win first; otherwise the resolver probes discovered candidates and prefers the newest usable executable, with the app-local `bin` directory as the first tie-breaker. Package-managed installations are detected and update commands are routed through Scoop, WinGet, Chocolatey, Homebrew, pip, or the detected standalone updater as appropriate. Install/update dialogs run with the app process environment, quote command paths containing spaces, close stdin to avoid helper prompts, can be cancelled by terminating the process tree, guard callbacks after dialog destruction, and surface permission-denied failures with specific guidance. Version probes use short watchdogs so broken aliases or hanging tools do not freeze the External Tools page. Retrieved standalone binaries are written atomically and verified against SHA-256 data when the upstream release publishes it.
- **Livestream and Replay Safety:** `DownloadManager`, `PlaylistExpansionParser`, runtime format metadata, and `YtDlpWorker` preserve yt-dlp `live_status` so active/upcoming livestreams use wait/finalization behavior while `post_live` and `was_live` replays are treated as archived media. Hostname-independent `/live/` URL-shape hints can bypass aria2c when extractor metadata is unavailable. Livestream wait intervals are clamped to a safe minimum and invalid max/min pairs are repaired before launch, livestream jobs bypass aria2c so yt-dlp's native wait/graceful interrupt behavior stays intact, and browser-cookie extractor failures retry once without `--cookies-from-browser` when public media can still be fetched.
- **Local API Server:** `LocalApiServer` is an optional, localhost-only `QTcpServer` on port `8765`. GUI mode starts it only when `General/enable_local_api` is enabled; `--server`, `--headless`, and `--background` start it automatically without changing that GUI preference. It generates or loads `api_token.txt` from the app-local data directory, using `Server/api_token.txt` for server/headless runtime isolation, requires `Authorization: Bearer <token>`, accepts `POST /enqueue` with `url` plus optional `type` (`video`, `audio`, or `gallery`) and optional caller-provided `id` (falling back to a generated UUID), and exposes `GET /status` snapshots sourced from download manager signals. Requests are bounded, empty/malformed request lines are rejected with JSON errors, `Content-Length`/`Expect`/Host/Origin parsing is anchored, and only localhost or trusted browser-extension origins are accepted. Token generation, unauthorized access, and valid enqueue parsing are covered by `TestLocalApiServer`. `MainWindowDiscord.cpp` also mirrors queue state to the local Discord bridge with compact sanitized status strings, preserved progress across partial queue refreshes, explicit playlist parent mapping, queue positions for queued jobs, terminal completion/cancellation states retained long enough for bridge clients to observe them, and bounded webhook requests whose callbacks are tied to the main window context and whose replies clean themselves up through Qt deferred deletion.
- **App Update Lookup:** `AppUpdater` checks the current lowercase GitHub repo slug first and can fall back to legacy repo API URLs so releases remain discoverable across repository renames. Version tags are parsed through the shared chronological `Version` helper before comparison, release JSON is validated before asset selection, malformed asset entries are skipped, GitHub rate-limit and not-found responses produce clear messages, and installer selection prefers `LzyDownloader-Setup-*.exe` assets.
...
3.  **Validation/Expansion:** `PlaylistExpansionWorker` validates the URL and checks for playlists asynchronously. It uses `YtDlpArgsBuilder` to construct the full yt-dlp command (including `--js-runtimes deno:...`, `--cookies-from-browser`, `--ffmpeg-location`, etc.) so playlist expansion matches the actual download configuration, while setting a builder option that prevents temp UUID directory creation during metadata-only expansion. `PlaylistExpansionParser` converts the resulting yt-dlp JSON into queue-ready item maps.
...
│   │   ├── BaseBinaryUpdater.h/cpp # Shared external binary update/version checks
│   │   ├── BinaryVerifier.h      # SHA-256 validation for downloaded binaries
│   │   ├── VersionParser.h       # Shared chronological version comparison helper
...
│   │   ├── AppUpdater.h/cpp      # Application update lookup/download logic
│   │   └── ...
...
- **Responsibilities:**
  - Provides the shared external-tool updater/checker used for `yt-dlp`, `gallery-dl`, and startup version probes for tools such as `ffmpeg` and `deno`.
  - Fetches local versions with bounded `QProcess` probes and caller-provided output parsers.
  - Checks GitHub release endpoints or direct latest-version endpoints, compares versions through `VersionParser`, and emits update status without blocking the UI.
  - Downloads standalone platform executable assets only when the discovered binary can be safely replaced, refuses package-managed overwrites, writes updates with `QSaveFile`, and verifies SHA-256 hashes when release metadata or `SHA256SUMS` assets provide them.
...
- **Responsibilities:**
  ...
  - Reads manual overrides directly from the app-local `settings.ini`, purges stale native-settings ghosts when the INI no longer contains an override, and returns invalid custom paths explicitly.
  - Searches the app-local `bin` folder, system `PATH`, and common Windows package-manager/user install folders.
  - Probes multiple candidates with short version checks and selects the newest executable, including date/snapshot-aware FFmpeg comparison.
...
- **Responsibilities:**
  ...
  - Detects livestream and live-replay options from queue metadata, preserving yt-dlp `live_status` so active/upcoming streams use wait/download container settings with safe wait interval bounds while `post_live`/`was_live` replays avoid livestream recorder args. Suppresses aria2c for true livestreams, finished live replays, and generic `/live/` URL-shape hints when metadata is unavailable.
