...
- **Responsibilities:**
  ...
  - Keeps startup and binary-resolution logic in `YtDlpWorker.cpp`, including bounded browser-cookie and transient aria2/native-downloader recovery retries plus the false-offline livestream retry path that can restart the worker once without `--wait-for-video` / `--live-from-start`.
