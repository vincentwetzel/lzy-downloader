...
- **Argument Construction**: Must dynamically build arguments based on all user settings, including:
    ...
    - Read-only expansion and referers: metadata-only playlist expansion must not add `--force-download`, even when duplicate/archive override is enabled. When aria2c is selected, a referer may be derived from the URL origin only if both scheme and host are present; incomplete URLs must omit the referer.
