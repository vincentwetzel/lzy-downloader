...
- **Advanced Settings Tab**:
    - **Organization**: Settings are grouped into logical sections:
        ...
        - **Authentication Access**: Cookies from browser (Video/Audio), Cookies from browser (Galleries). The cookie access check is handled directly within `AdvancedSettingsTab` using `QProcess`, with a 30-second timeout and improved logging. The check uses a specific YouTube Shorts URL for more reliable validation, and validation probes intentionally omit `--cookies-from-browser` so they stay lightweight and avoid browser-profile locking.
        ...
        - **Metadata / Thumbnails**: Embed metadata, Embed thumbnail, Use high-quality thumbnail converter, Convert thumbnails to, Force Playlist as Single Album, Generate `folder.jpg` for explicit full audio playlist batches.
...
- **Platform-Specific Updater Assets**: `AppUpdater` must select the downloaded update artifact that matches the current OS (`.exe` on Windows, `.AppImage` on Linux, `.dmg` on macOS) and launch it with the correct platform-specific handoff behavior after download.
- **Argument Construction**: Must dynamically build arguments based on all user settings, including:
    ...
    - Filename restriction (`--restrict-filenames`).
      - External downloader (`--external-downloader aria2c`) for ordinary non-livestream downloads only; active/upcoming livestreams and completed live replays (`post_live`/`was_live`) stay on yt-dlp's native downloader so wait-state, replay manifests, and graceful finish handling remain reliable. A `/live/` URL path segment is not itself a livestream signal. When aria2c is selected, the app should forward a generic referer derived from the request URL origin so hosts that require referer-gated segmented transfers can complete without site-specific overrides.
    ...
    - Browser-cookie fallback: if yt-dlp fails while using browser cookies because cookie extraction is denied or cookie-backed extractor state incorrectly reports public media as unavailable/ended, the worker may retry once without `--cookies-from-browser` and must surface clear diagnostics if the retry still fails. Retry detection must require explicit cookie/browser-database/sign-in evidence; generic words such as `locked` in a title or description are not cookie diagnostics and must not kill the worker or leave the item active indefinitely.
    ...
    - URL hygiene: before launching yt-dlp, the argument builder strips common tracking/query parameters such as `utm_*`, `enter_*`, `igshid`, `fbclid`, `si`, `ref`, and `source` when present, so transient sharing parameters do not affect downloader behavior or archive identity. The cleaned URL uses Qt's standard URL serialization.
    - Read-only expansion and referers: metadata-only playlist expansion must not add `--force-download`, even when duplicate/archive override is enabled. Playlist-expansion probes also skip `--cookies-from-browser` so URL validation stays non-interactive and read-only. When aria2c is selected, a referer may be derived from the URL origin only if both scheme and host are present; incomplete URLs must omit the referer.
...
- **Bounded Diagnostics**: yt-dlp and gallery-dl workers must retain only a bounded tail of warning/error output for terminal diagnostics so long-lived or noisy jobs do not grow memory without limit.
...
- **Qt 6.2 Compatibility**: Helper code that targets Qt 6.2 must prefer Qt-native string and character comparisons over newer `QStringView` conveniences that are not available on older minor releases.
...
- **File Lifecycle:**
    ...
    - **Resuming and Clearing:** Partially downloaded files are retained in the temporary directory when a download is stopped. `DownloadQueueState` persists the current `tempFilePath`, `originalDownloadedFilePath`, and tracked cleanup candidates so users can resume or manually clean up across application restarts. Terminal finalization removes the guarded per-download UUID directory even when output discovery, gallery validation, or final-destination replacement fails.
    ...
    - A file stability check must be performed before moving.
      - Files are moved to a final destination directory upon success. FFmpeg single-input finalization should prefer a direct move/rename, but if that fails because of a cross-volume or filesystem-boundary condition, the app may fall back to copy/remove so the download still completes cleanly.
...
- **Release Packaging Automation**: `build_release.py` is the canonical release builder. It must parse the version from `CMakeLists.txt`, refresh extractor JSON files, clean/configure/build `build-release`, package `LzyDownloader-Setup-X.X.X.exe` on Windows via NSIS, and package `LzyDownloader-X.X.X-x86_64.AppImage` on Linux via linuxdeploy. Linux AppImage packaging must use a clean `build-release/AppDir` staging directory and generate a release desktop file whose `Icon` entry matches the resized PNG passed to linuxdeploy so root AppDir icon deployment succeeds on WSL and CI. The GitHub Actions release workflow must trigger on `v*` tags and upload those platform assets to the matching GitHub Release. Qt setup must rely on the installer action's implicit Qt Base package for Widgets/Core/Network/Sql and request only add-on modules from `aqt` when the project actually links one. Windows CI should use the Qt 6.6 `win64_msvc2019_64` desktop archive unless the project intentionally moves to a Qt version that publishes a newer MSVC archive. The helper inherits the caller's terminal streams and must not open a separate Windows command window.
