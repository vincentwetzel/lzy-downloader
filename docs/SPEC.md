...
- **Binary Discovery & Enforcement**: On launch, the application must search for required external binaries in manual overrides, the app-local `bin` directory, system `PATH`, and common package-manager or user-local locations. If multiple candidates are present, it should choose the newest usable executable with bounded version probes while preserving manual overrides as the highest priority. The application must actively prevent the user from queuing video/audio downloads if `yt-dlp`, `ffmpeg`, `ffprobe`, or `deno` are missing, and block gallery downloads if `gallery-dl`, `ffmpeg`, or `ffprobe` are missing. Interactive missing-binary flows must open a guided setup dialog that lists the missing tools, offers install and browse actions, and re-scans status in place instead of only directing users to the Advanced Settings page.
- **Binary Management UI**: The External Binaries page must show the detected/configured path and current version for each supported binary, plus update warnings when startup/version checks detect a newer available release. In-app binary updates must refuse to overwrite package-managed installations and must verify SHA-256 hashes when upstream release metadata publishes them.
- **Application Updates**: GitHub release checks must validate JSON object/array types before reading release notes or assets, skip malformed asset entries, parse chronological/date-like and semantic version tags through the shared version parser, distinguish rate-limit/not-found failures, and report a clear update-check failure if a newer release has no usable assets.
...
- **Application Update UX**: Startup update checks must wire `AppUpdater` signals to a user prompt, compare semantic and date-like version tags with the shared chronological parser, surface GitHub rate-limit and missing-repository errors clearly, and prefer installer assets named `LzyDownloader-Setup-*.exe`.
- **Dependency Management**:
  ...
  - Update commands for package-managed binaries must call the relevant package manager instead of overwriting files directly; standalone binaries may use their own updater command such as `yt-dlp -U`, `gallery-dl -U`, or `deno upgrade`, or the shared in-app updater when the executable can be safely replaced.
