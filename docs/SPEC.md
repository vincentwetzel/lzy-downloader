...
- **Release Packaging Automation**: `build_release.py` is the canonical release builder. It must parse the version from `CMakeLists.txt`, refresh extractor JSON files, clean/configure/build `build-release`, package `LzyDownloader-Setup-X.X.X.exe` on Windows via NSIS, and package `LzyDownloader-X.X.X-x86_64.AppImage` on Linux via linuxdeploy. The GitHub Actions release workflow must trigger on `v*` tags and upload those platform assets to the matching GitHub Release.
